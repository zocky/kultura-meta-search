...
var clientPaths = {
	"/": {
		file: "core/client/index.html",
		type: "text/html"
	},
	"/robots.txt": {
		file: "core/server/robots.txt",
		type: "text/plain"
	}
}
...
// TODO: replace with a proper server, probably

function onHttpRequest (req,res) {
	var u = url.parse(req.url,true);
	if (u.pathname == '/' && u.query.q == 'conf') {
	...
	} else if (u.pathname.match(/^[/](js|css)[/]/)) {
		sendFile(res,'core/client/' + u.pathname);  
	} else if (u.pathname.match(/^[/](sta[t]ic)[/]/)) {
		sendFile(res,'local' + u.pathname); 
	} else {
		res.writeHead(404);
